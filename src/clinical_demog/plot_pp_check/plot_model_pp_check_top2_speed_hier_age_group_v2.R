...
fit_name <- "speed_hier_age_group_v2"
fit <- fits[[fit_name]]

subtitle <- TeX(paste0("Hierarchical SPEED grouped by AGE-GROUP v2"))
...
plot_factors <- fit |> 
  ...
  labs(
    ...
    y = "",
    caption = "Note: Higher values indicate increased fall risk."
  ) +
...
ggsave(
  paste0(c(plots_path, paste0("factors", "_", fit_name, ".png")), collapse = "/"),
  plot_factors,
  width = 7,     # inches
  height = 5,     # inches
  dpi = 300,      # high resolution
...
tidybayes::get_variables(fit)[1:10]
...
# First, let's check the actual range of values
fit |> 
  tidy_draws() |> 
  select(starts_with("b_")) |>
  pivot_longer(cols = everything()) |>
