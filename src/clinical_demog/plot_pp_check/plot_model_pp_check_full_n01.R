...
subtitle <- TeX(paste0(gsub("_", " ", gsub("c-", "", fit_name)), " - ", "$N(0,1)$"))
...
ggsave(
  paste0(c(plots_path, paste0("pp_check", "_", prior_suffix, "_", fit_name, ".png")), collapse = "/"),
  pp_plot,
  width = 7,     # inches
  height = 5,     # inches
...
plot_factors <- fit |> 
  ...
  mutate(
    ...
    name = case_match(
      ...
      "FSST" ~ "Four Square Step Test (FSST)",
      "GCS_NEUROTRAX" ~ "Global Cognitive Score (GCS)",
      "GENDER" ~ "Gender",
      "log_TMT_B" ~ "Trail Making Test B (TMT-B)",
      "TUG" ~ "Timed Up & Go (TUG)",
      "Intercept" ~ "Intercept"
    ),
  ...
  labs(
    ...
    y = "",
    caption = "Note: Higher values indicate increased fall risk. GDS = Geriatric Depression Scale; DGI = Dynamic Gait Index; TMT-B = Trail Making Test B (log-transformed)"
  ) +
...
ggsave(
  paste0(c(plots_path, paste0("factors", "_", prior_suffix, "_", fit_name, ".png")), collapse = "/"),
  plot_factors,
  width = 10,     # inches
  height = 5,     # inches
  dpi = 300,      # high resolution
